...
async function getNotificationPermissionAndToken(userId:string) {
  ...
  if (Notification.permission === "granted") {
    return await fetchToken(userId);
  }
...
const useFcmToken = (userId:string) => {
  ...
  const loadToken = async () => {
    ...
    const token = await getNotificationPermissionAndToken(userId); // Fetch the token.
    ...
    const setupListener = async () => {
      ...
      const unsubscribe = onMessage(m, (payload) => {
        ...
        n.onclick = (event) => {
          ...
          const target = event.target as HTMLElement & { data?: { url?: string } };
